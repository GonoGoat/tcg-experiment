 ...
 * @param maximum Maximum allowed numeric value
 * @param setter Handler used to set the value to the state
 * @param symbol Mathematic operator to indicate as prefix to the value
 */
export const handleNumericChangeForInequalities = (value: string, maximum: number, setter: (val: string) => void, symbol: string) => {
    if (value === "") {
        setter("")
    }
    else if (!isNaN(Number(value))) {
        if (parseInt(value) <= maximum) setter(`${symbol}${value}`)
    }
}